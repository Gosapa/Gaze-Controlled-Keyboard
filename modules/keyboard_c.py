...
class Keyboard:
    ...
    def select(self):
        self.update_status()
        returned_characters, newSpecialValue = character(self.status, self.isSpecialEntered)
        print("(DEBUG) returned_characters: " + str(returned_characters))
        self.isSpecialEntered = newSpecialValue
        if newSpecialValue:
            print("(DEBUG) Total List appended with special value")
    ...
    def enter(self, character):
        ...
        # "ㄲ"
            ...
            PressKey(0x10)
            PressKey(KOREAN_KEYBOARD_HEX[0])
            ReleaseKey(0x10)
            ReleaseKey(KOREAN_KEYBOARD_HEX[0])
    ...
    def part_selections(self):
        ...
        for i in range (0,2):
            ...
            status_tmp[0] += 1
            newList, _ = character(status_tmp, self.isSpecialEntered)
            self.selection_list.append(newList)
    ...
    def draw_second(self):
        ...
        if self.status[1] == 0:
            ...
            draw_boxes(self.screen, box_num, width, height, self.cur_selection)
            put_letter_font(self.screen, "ㅏ~ㅜ", (0, 200), 250)
            put_letter_font(self.screen, "ㅠ~ㅣ", (650, 200), 250)
    def draw_bsearch(self):
        box_num = len(self.total_list)
        isConsonant = self.status[1] == 0
        width = 150 if isConsonant else 200
        height = 150 if isConsonant else 200
        draw_boxes_bsearch(self.screen, self.total_list, width, height, self.selection_list[self.cur_selection], self.active_list)
        for i in range(box_num):
            start_offset = 10 if isConsonant else 40
            xpos = i * 179 if isConsonant else i*250
            ypos = 250
            font_size = 180 if isConsonant else 180
            put_letter_font(self.screen, KOREAN_LIST[self.total_list[i]], (start_offset + xpos, ypos), font_size)
    def draw_special(self):
        draw_boxes(self.screen, 2, 600, 450, self.cur_selection)
        put_letter_font(self.screen, KOREAN_LIST[self.total_list[0]], (20, 200), 250)
        put_letter_font(self.screen, KOREAN_LIST[self.total_list[1]], (650, 200), 250)
